...
class ThreadOwnerTracker:
    """Persistent mapping of thread IDs to the default responding bot/profile.

    Discord multi-agent channels need stronger routing than simple
    participation tracking: once one bot creates/owns a thread, another bot
    that is explicitly mentioned later may participate without becoming the
    mention-free default responder for future replies. This helper stores that
    owner/default-responder mapping separately from ThreadParticipationTracker.
    """
            ...
            try:
                ...
                if isinstance(data, dict):
                    return {
                        ...
                        if str(thread_id) and str(owner)
                    }
            except Exception:
                pass
        return {}
    ...
    def mark_owner(self, thread_id: str, owner: str) -> None:
        """Record *owner* as the default responder for *thread_id*.

        Existing owners are intentionally preserved so participation by another
        bot cannot silently transfer mention-free ownership.
        """
        thread_id = str(thread_id)
        owner = str(owner)
        if not thread_id or not owner or thread_id in self._owners:
            return
