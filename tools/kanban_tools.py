...
def _handle_submit_review(args: dict[str, Any], **kw) -> str:
    ...
    try:
        ...
        try:
            ...
            task = kb.get_task(conn, tid)
            run = kb.latest_run(conn, tid)
            review_watch = _maybe_auto_subscribe_info(conn, tid)
            return _ok(
                task_id=tid,
                status="review",
...
def _handle_submit_result(args: dict[str, Any], **kw) -> str:
    ...
    try:
        ...
        try:
            ...
            if not ok:
                return tool_error(
                    f"could not submit result for {tid} (unknown id, invalid "
                    f"reviewer, not reviewable, or stale worker run)"
                )
            task = kb.get_task(conn, tid)
            run = kb.latest_run(conn, tid)
        ...
        try:
            ...
            if not ok:
                return tool_error(
                    ...
                    f"submission event, not running review, or stale worker run)"
                )
            task = kb.get_task(conn, tid)
            run = kb.latest_run(conn, tid)
            return _ok(
...
def _maybe_auto_subscribe(conn: Any, task_id: str) -> bool:
    ...
    return bool(_maybe_auto_subscribe_info(conn, task_id).get("attached"))


def _maybe_auto_subscribe_info(conn: Any, task_id: str) -> dict[str, Any]:
    """Best-effort notification subscription with caller-visible details.

    Used by ``kanban_create`` (as a boolean) and by D2-f review submissions
    (as ``review_watch``) so async review outcomes can route back to the
    originating gateway/session source. CLI / cron / unattached contexts
    deliberately return ``{"attached": False}`` instead of falling back to
    ``HERMES_SESSION_ID``.
    """
    try:
        cfg = load_config()
        ...
        # If config can't load we still default to True — this mirrors the
        # pre-gate implementation and keeps notification setup best-effort.
        pass
    ...
    try:
        ...
        if not platform or not chat_id:
            # TUI / desktop fallback. Do NOT use HERMES_SESSION_ID here:
            # it is also set for CLI/ACP telemetry and would over-subscribe
...
KANBAN_SUBMIT_RESULT_SCHEMA = {
    ...
    "description": (
        "Submit your current task result to a creator/acceptor while keeping "
        "the same task id. This closes your current run as "
        "submitted_result/released, records result evidence, clears the "
        "claim, and moves the card to review for the acceptor. Use this when "
        "your deliverable is ready but must be accepted before the task can "
        "be final done. The acceptor can kanban_complete to accept or "
        "kanban_request_changes to return the same card to you."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            ...
            "reviewer": {
                "type": "string",
                "description": (
                    "Creator/acceptor profile/assignee. If omitted, the "
                    "task's created_by profile is used only when it resolves "
                    "to a real spawnable profile."
                ),
            },
            "summary": {
                "type": "string",
            ...
            "metadata": {
                "type": "object",
                "description": "Optional structured result evidence.",
            },
            "board": _board_schema_prop(),
        },
        "required": [],
