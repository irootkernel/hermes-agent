...
def test_recovery_preserves_messages_with_cjk_index_enabled(
    ...
    assert report["copy"]["messages"]["status"] == "complete", report["copy"][
        "messages"
    ].get("error")
...
def test_lost_and_found_recovery_preserves_messages_with_cjk_index_enabled(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    recovery_cjk_so: Path,
) -> None:
    ...
    def fake_recover(
        ...
        try:
            ...
            recovered_rows.create_session("cjk-lost-and-found-session", "cli")
            recovered_rows.append_message(
                "cjk-lost-and-found-session",
                "user",
                "한국어 페이지 복구 경로를 확인한다",
            )
        finally:
