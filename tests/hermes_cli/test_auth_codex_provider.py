...
def test_save_codex_pool_entry_updates_only_matching_label(tmp_path, monkeypatch):
    ...
    assert auth["providers"]["openai-codex"]["tokens"]["access_token"] == "jyh-singleton-at"


def test_save_codex_pool_entry_creates_missing_label(tmp_path, monkeypatch):
    hermes_home = tmp_path / "hermes"
    hermes_home.mkdir(parents=True, exist_ok=True)
    (hermes_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {"openai-codex": {"tokens": {"access_token": "singleton-at"}}},
        "credential_pool": {"openai-codex": []},
    }))
    monkeypatch.setenv("HERMES_HOME", str(hermes_home))

    entry = _save_codex_pool_entry(
        {"access_token": "new-at", "refresh_token": "new-rt"},
        last_refresh="2026-06-17T15:00:00Z",
        label="HSY",
    )
    ...
    assert pool[0]["source"] == "manual:device_code"
    assert pool[0]["access_token"] == "new-at"
    assert auth["providers"]["openai-codex"]["tokens"]["access_token"] == "singleton-at"


def test_save_codex_pool_entry_duplicate_label_fails_closed(tmp_path, monkeypatch):
    hermes_home = tmp_path / "hermes"
    hermes_home.mkdir(parents=True, exist_ok=True)
    (hermes_home / "auth.json").write_text(json.dumps({
        "version": 1,
        ...
        "credential_pool": {
            "openai-codex": [
                {"id": "a", "label": "HSY", "source": "manual:device_code"},
                {"id": "b", "label": "HSY", "source": "device_code"},
            ],
        },
    }))
    monkeypatch.setenv("HERMES_HOME", str(hermes_home))
    ...
    assert exc.value.code == "codex_label_ambiguous"
