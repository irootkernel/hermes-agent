...
KANBAN_GUIDANCE = (
    ...
    "5. **Complete or submit result.** Use `kanban_complete(summary=..., metadata=...)` "
    "only when this run may mark the card done. If a creator/acceptor must "
    "approve your work, use `kanban_submit_result(reviewer=<creator>, summary=..., "
    "metadata=...)`; it closes as `submitted_result`, moves the same card to "
    "`review`, and the acceptor can complete or `kanban_request_changes` it. "
    "Use `kanban_submit_review` only for explicit peer-review cards. Keep "
    "summary short, metadata structured, and secrets/PII out. If review is "
    "needed but no acceptor is known, put evidence in `kanban_comment` then "
    "`kanban_block(reason=\"review-required: <one-line summary>\")`. Reviewing-then-"
