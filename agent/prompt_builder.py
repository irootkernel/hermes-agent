...
KANBAN_GUIDANCE = (
    ...
    "5. **Hand off without creating a new card.** When another profile owns the "
    "next step, call `kanban_reassign(assignee=..., summary=..., metadata=...)`. "
    "This keeps the same task id and same workspace, releases your active run, "
    "and routes the card to that assignee. Stop immediately after success; do "
    "not continue working from the released run.\n"
    "6. **Use native same-card peer review.** An implementer calls "
    "`kanban_submit_review(reviewer=..., final_assignee=..., summary=..., "
    "metadata=...)` when an independent reviewer must inspect the work. The "
    "reviewer calls `kanban_request_changes(reason=..., assignee=...)` to return "
    "the same card for rework, or `kanban_complete(summary=...)` to accept it. A "
    "distinct final assignee receives the accepted card for final completion; "
    "without one, review acceptance completes the card.\n"
    "7. **Submit finished implementation for acceptance.** When implementation "
    "is complete and the task creator or a distinct acceptor owns the acceptance "
    "decision, call `kanban_submit_result(reviewer=..., summary=..., metadata=..., "
    "artifacts=[...])`. The acceptor calls `kanban_request_changes(...)` for "
    "rework or `kanban_complete(...)` to accept the result. Every successful "
    "handoff, review, result submission, or change request releases the current "
    "run, so stop immediately after success.\n"
    "8. **Complete with structured handoff.** Call `kanban_complete(summary=..., "
    "metadata=...)` only when your current gate is genuinely finished. `summary` "
    "is 1–3 human-readable sentences naming concrete artifacts. `metadata` is "
    "machine-readable facts (`{changed_files: [...], tests_run: N, decisions: "
    "[...]}`). Downstream workers read both via their own `kanban_show`. Never put "
    "secrets / tokens / raw PII in either field — run rows are durable forever.\n"
    "9. **If follow-up work appears, create it; don't do it.** Use "
    ...
    "- **Deliverables.** Files a human wants go in the active terminal path: "
    "implementers use `kanban_submit_result(artifacts=[<absolute paths>])` so "
    "accepted scratch outputs survive cleanup; final-only workers use "
    "`kanban_complete(artifacts=[<absolute paths>])`. Paths in `metadata` alone "
    "are NOT uploaded. Files must exist when the task is finally accepted.\n"
