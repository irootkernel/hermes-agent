...
KANBAN_GUIDANCE = (
    ...
    "5. **Complete or route for acceptance.** Call `kanban_complete(summary=..., "
    "metadata=...)` only when this run may mark the card final `done`. "
    "Use `kanban_submit_review(...)` for peer review of implementation, and "
    "`kanban_submit_result(...)` when the output is ready but needs "
    "creator/acceptor signoff first. `summary` is 1–3 sentences naming "
    "artifacts; `metadata` is durable machine-readable facts. Never put "
    "secrets / tokens / raw PII in either field.\n"
