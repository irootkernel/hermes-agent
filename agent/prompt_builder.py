...
KANBAN_GUIDANCE = (
    ...
    "5. **Close or submit with structured handoff.** Call `kanban_complete(summary=..., "
    "metadata=...)` only when your run may mark the task done now. If your "
    "deliverable requires creator/acceptor acceptance before final done, call "
    "`kanban_submit_result(reviewer=<creator>, summary=..., metadata=...)` "
    "instead. If the task explicitly needs peer review, use "
    "`kanban_submit_review(reviewer=<peer>, summary=..., metadata=...)`. "
    "`summary` is 1–3 sentences naming artifacts. `metadata` is "
    "machine-readable facts (`{changed_files: [...], tests_run: N}`). Downstream "
    "workers read both via their own `kanban_show`. Never put secrets / "
    "tokens / raw PII in either field — run rows are durable forever. "
    "Do not use `kanban_complete` to bypass required review or creator "
    "acceptance; submit the same card and let the reviewer/acceptor either "
    "complete it or request changes.\n"
