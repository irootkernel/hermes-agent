...
KANBAN_GUIDANCE = (
    ...
    "6. **Use native same-card review.** An implementer calls "
    "`kanban_submit_review(reviewer=..., final_assignee=..., summary=..., "
    "metadata=...)` instead of blocking or creating a review card. The reviewer "
    "calls `kanban_request_changes(reason=..., assignee=...)` to return the same "
    "card for rework, or `kanban_complete(summary=...)` to accept it. A distinct "
    "final assignee receives the accepted card for final completion; without one, "
    "review acceptance completes the card. Every successful transition releases "
    "the current run, so stop immediately.\n"
