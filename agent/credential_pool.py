...
def get_credential_pin(provider: str, config: Optional[Dict[str, Any]] = None) -> Dict[str, str]:
    ...
    Credential pins are read only from the profile-local .env file so host-local
    account assignment survives config.yaml schema migrations and cannot bleed
    across profiles through a parent process environment::

        HERMES_CREDENTIAL_PIN_OPENAI_CODEX_LABEL=HSY

    ``_ID`` takes precedence over ``_LABEL``. A bare provider env var, such as
    ``HERMES_CREDENTIAL_PIN_OPENAI_CODEX=HSY``, is treated as a label for
...
class CredentialPool:
    ...
    def _resolve_pinned_entry(
        ...
        Matching by id is authoritative. Label matching is exact and must be
        unique across the full pool, not just currently available entries, so an
        unavailable duplicate cannot silently choose the other account. When
        *candidates* is provided, the resolved entry must also be present in
        that available set; otherwise the pin fails closed.
        """
        if not pin:
        ...
        matches = [
            ...
            if str(item.label or "").strip() == pinned_label
        ]
        if not matches:
