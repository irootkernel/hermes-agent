...
def _credential_pin_env_prefix(provider: str) -> str:
    """Return the provider-specific env prefix for credential pins."""
...
def _credential_pin_env_value(key: str) -> str:
    """Read a credential-pin variable from the profile-local .env file only.

    Process-level environment variables are intentionally ignored here: gateway
    parents can carry another profile's environment, and credential account
    assignment must not bleed across profiles.
    """
    try:
...
def get_credential_pin(provider: str, config: Optional[Dict[str, Any]] = None) -> Dict[str, str]:
    """Return the configured credential pin for *provider*.

    Credential pins are intentionally read only from the profile-local .env file
    so host-local account assignment survives config.yaml schema migrations and
    cannot bleed across profiles through a parent process environment::

        HERMES_CREDENTIAL_PIN_OPENAI_CODEX_LABEL=JYH

    ``_ID`` takes precedence over ``_LABEL``.  A bare provider env var, such as
    ``HERMES_CREDENTIAL_PIN_OPENAI_CODEX=JYH``, is treated as a label for
    convenience.  Empty or unset env values return an empty dict so callers keep
    existing pool strategy behavior.
    """
    _ = config  # API compatibility; config.yaml credential pins are unsupported.
...
def has_configured_credential_pin(provider: str, config: Optional[Dict[str, Any]] = None) -> bool:
    """True when env pins *provider* to a credential id or label."""
...
class CredentialPool:
    ...
    def configured_pin(self) -> Dict[str, str]:
        """Return this provider's configured credential pin, if any."""
        return get_credential_pin(self.provider)

    def has_configured_pin(self) -> bool:
        """True when this provider is pinned to a credential id or label."""
    ...
    def _resolve_pinned_entry(
        ...
        Matching by id is authoritative.  Label matching is case-insensitive but
        must be unique across the full pool, not just currently available
        entries, so an unavailable duplicate cannot silently choose the other
        account.  When *candidates* is provided, the resolved entry must also be
        present in that available set; otherwise the pin fails closed.
        """
        if not pin:
        ...
        if not pinned_label:
            return None, "credential pin must specify id or label"
        label_key = pinned_label.lower()
        matches = [
            item for item in self._entries
            if str(item.label or "").strip().lower() == label_key
        ]
        if not matches:
            return None, f'pinned credential label "{pinned_label}" not found'
        if len(matches) > 1:
            return None, f'pinned credential label "{pinned_label}" is ambiguous'
        entry = matches[0]
        if available_ids is not None and entry.id not in available_ids:
    ...
    def _select_unlocked(self) -> Optional[PooledCredential]:
        ...
        if pin:
            entry, error = self._resolve_pinned_entry(pin, available)
            if entry is None:
                self._current_id = None
                logger.warning(
