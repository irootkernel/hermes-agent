...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    p_submit_result = sub.add_parser(
        "submit-result",
        help="Submit a task result to a creator/acceptor for final acceptance",
    )
    p_submit_result.add_argument("task_id")
    p_submit_result.add_argument(
        "--reviewer",
        default=None,
        help="Creator/acceptor profile. Defaults to task created_by when spawnable.",
    )
    p_submit_result.add_argument(
        "--summary",
        default=None,
    ...
    p_submit_result.add_argument(
        "--metadata",
        default=None,
        help="JSON dict of structured result evidence.",
    )
...
def _cmd_submit_result(args: argparse.Namespace) -> int:
    ...
    with kb.connect_closing() as conn:
        ...
        if not ok:
            print(
                ...
                "(unknown id, invalid reviewer, not reviewable, or stale worker run)",
                file=sys.stderr,
            )
            return 1
        task = kb.get_task(conn, args.task_id)
    ...
    if getattr(args, "json", False):
        ...
        assignee = task.assignee if task else reviewer_name
        print(f"Submitted result for {args.task_id} to {assignee}")
    return 0
